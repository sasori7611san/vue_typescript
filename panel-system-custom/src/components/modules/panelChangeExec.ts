...
// パネル更新（左方向）（col:対象色番号,v:縦番号,s:横番号,pan:パネル,color:使用色番号,total:パネル集計,sheet:パネル変化枚数）
...
const panelChangeExec = (col: number, v: number, s: number, pan: Panel[][], total: Total): void => {
  colorSet(col, v, s, pan)
  panelAggregation(pan, total)
}
// パネル動作(時間考慮)（col:対象色番号,v:縦番号,s:横番号,pan:パネルtotal:パネル集計）
